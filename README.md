...
- 손님 NPC
  ...
  - `CustomerOrderAndWait` : 주문/대기/수령 처리
  - `CustomerMoveToTable` : 좌석 탐색/이동 + 좌석 없을 때 주기적 재탐색
  - `CustomerEating` / `CustomerGoingHome` : 식사 / 퇴장 처리
- 길찾기
  - `AStarPathfinder` : A* 경로 탐색
  - `Node` / `NodeManager` : 노드 맵 구성 및 연결 관리
- 상호작용/오브젝트
  - `ObjectInteration` : 트리거 기반 상호작용 제어
  - `ObjectPooling` : 오브젝트 재사용(풀링) 관리
...
## 🧯 트러블슈팅
### 1) 좌석이 꽉 차면 손님 흐름이 끊기는 문제
- 문제: 좌석이 없을 때 손님이 멈추거나 진행이 끊기는 현상
- 원인: 좌석이 없으면 목적지 갱신/재탐색이 발생하지 않음
- 해결(내가 적용): 좌석이 없을 때 대기 상태로 전환하고, 일정 주기로 좌석을 재탐색하여 경로를 재계산하도록 개선
- 결과: 좌석이 비는 즉시 자연스럽게 착석 흐름으로 복귀

### 2) 다수 손님 이동 시 겹침/끼임이 발생하는 문제
- 문제: 손님 수가 늘면 경로가 겹치면서 서로 겹치거나 끼이는 현상
- 원인: 동일한 경로/노드로 동시에 진입하면서 충돌이 발생
- 해결(내가 적용): 노드 점유 개념을 도입해 이동 중 충돌을 줄이도록 개선(노드 점유/해제 흐름을 기준으로 이동 제어)
- 결과: 밀집 상황에서도 손님 이동이 더 안정적으로 동작
...
- Notion: https://www.notion.so/1d68c79bfc3f80059f14cd13f8e2000a?source=copy_link
